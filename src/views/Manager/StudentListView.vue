...
import EditStudentDialog from "@/components/Manager/EditStudentDialog"
import Paginator, { type PageState } from "primevue/paginator"

export type IDictionaryItemName = 'regions' | 'groups' | 'cities' | 'managers'

const edit_dialog_visible = ref(false)
const editable_student = ref<IUser>()
...
const selected_manager = ref(null)
...
const getDictionaryItemList = async() => {
  ...
  managers.value  = await getDictionaryItem('managers')
  ...
  try {
    const { data } = await http.get('get-students', {
      params: {
        ...
        ...(selected_manager.value ? { managerId: selected_manager.value } : {})
      }
    })
...
const openEditDialog = (data: IUser) => {
  edit_dialog_visible.value = true
  editable_student.value = data
}

const closeEditDialog = () => {
  editable_student.value = undefined
}
...
<template>
  <template v-if="editable_student">
    <EditStudentDialog v-model="edit_dialog_visible" :student="editable_student" @close="closeEditDialog"/>
  </template>
  <div>
    <h1 class="font-semibold">Все студенты</h1>
    <div class="mt-5 mb-4">
