...
const show_course_schedule_form = ref(-1)
...
const getTeacherList = async() => {
  try {
    const { data } = await http.get(`admin/get-teachers-by-subject/${selected_course.value?.id}`)
    teachers.value = data || []
  } catch(e) {
    console.error(e)
    throw e
...
const createClass = async() => {
  ...
  try {
    const { data } = await http.post("admin/subgroups", {
